...
function (p::harmonic_pol)(x::Real)
    Q_0, Q_1,Q_2 = p.coeff
    period = p.period
    return Q_0 + Q_1*cos(2*pi*x/period) + Q_2*sin(2*pi*x/period)
end
...
function fit!(pol::harmonic_pol, x::Vector{Float64}, y::Vector{Float64})
    ...
    # |1 * *|
    # |1 * *|
    # Determining the size of x to create a matrix with a corresponding number of rows
    size_ = size(x)
    ...
    # Calculating X^TX and X^Ty, essential steps for solving the normal equation
    XtX = X_feature' * X_feature  # Transposing  of X_feature multiplied by X_feature
    Xty = X_feature' * y          # Transposing of X_feature multiplied by y
    # DO NOT USE "\" operation!!! 
      XtX_inv = inv(XtX)
    # your code here
    # the coefficients that minimize ||XV - y||^2.
      V = XtX_inv * Xty
     # Updating the coefficients of the polynomial model with the newly computed vector V.
     pol.coeff = V
end
